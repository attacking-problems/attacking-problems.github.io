...
.Under Construction
====
The new version of the book was created during the summer of 2024 and is now usable.  We are continuing to work on additional changes, noted below.  Please let us know of any issues you spot--or of any changes you would particularly like to see.
====

=== Changes Planned and Completed

* [ ] Add discussion of the `record` feature to Classes (Chapter 9) and how they can be used in `switch` statements to Polymorphism (Chapter 18). 
...
* [x] Include an explanation of the `enum` type in Classes (Chapter 9).
* [x] Unify discussion of annotations (Chapters 13 and 17).
* [x] Add an explanation of _varargs_ in Methods (Chapter 8).
* [x] Improve `switch` examples in Selection (Chapter 4).  Add `yield` example(s).
* [x] Describe improvements to the `interface` specification, including `static` methods and `default` methods.
* [x] Move the introduction of `instanceof` from Interfaces (Chapter 10) to Polymorphism (Chapter 18).  Add binding variable and `switch` examples.
