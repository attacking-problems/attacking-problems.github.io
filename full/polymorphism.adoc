...
One such tool is abstract classes. An abstract class is one that can
...
Recall from <<Syntax: Interfaces>> that methods in interfaces are usually
abstract. You can also add abstract methods to abstract classes. In any abstract
class, you can create a method header describing a method that all non-abstract
child classes must implement, as shown below.
...
The `Sequence` class doesn't specify _how_ the sequence of numbers
...
since each type of bank account has unique characteristics, we don't want people
...
constants before. The meaning `final` has for methods is similar to
what it means for constants (and almost the opposite of `abstract`). A method
which is declared `final` _can't_ be overridden by child classes. If you're
designing a class hierarchy and you want to lock a method into doing a
specific thing and never changing, this is the appropriate tool.
...
This process could be arbitrarily more complex. We could be reading data
...
important thing. Now, we can run through the array with a loop, and 
the code will magically work for each different kind of `Vehicle`.
...
With these casts, both lines of code will work. The compiler is always
conservative. It never makes guesses about the type of something. Consider the
following.
...
is a `RocketShip`, we still must perform an explicit cast. Note that
`instanceof` looks at the object itself; thus, a reference that points to `null`
has no object, no type, and will give always give a `false` result in an
`instanceof` expression.

Sometimes `instanceof` is not precise enough. If you must be sure that the object
in question has a particular class and not just one of its child classes, you
...
Java 14 added an optional _binding variable_ to the `instanceof` keyword. If
the `instanceof` expression is inside an `if` statement, a binding variable can
be added after the type. If the object matches the type, this binding variable
will hold a reference to that object, automatically stored as
the new type without the need for an explicit cast. Using this updated syntax,
we could write our first `instanceof` example as follows.

[source,java]
----
public void visitDenver(Vehicle vehicle) {
    if (vehicle instanceof RocketShip ship) {
        ship.blastOff();
    }
    vehicle.travel("Denver");
}
...
This style is preferred when possible, since it uses less code and removes the
explicit cast.
 

The `instanceof` keyword can be used with interfaces as well. For example, the
following method takes a `Chord` object and an object of type `Object`.
Recall that all reference types in Java can be treated as an `Object` type, so
a reference of type `Object` could point at any object of any type.
In this case, if the `Object` turns out to implement the `Guitarist` interface,
we can bind it to a `Guitarist` variable called `player` and use it to strum a
chord.

[source,java]
----
void checkBeforeStrum(Object unknown, Chord chord) {
    if (unknown instanceof Guitarist player) {
...
if you use a `Plum` in place of a `Fruit`, calling the `eat()` method shouldn't
cause an issue. A problem would only happen if a `ChokingOnPitException` is thrown.
...
`final` because an animal will never switch between warm-blooded and
cold-blooded).
...
=== Advanced: Pattern matching on types

The `instanceof` keyword can always be used to see if an object has a particular
type (or a child of that type), but a powerful alternative is to use pattern
matching in `switch` statements, a set of tools added in Java 21. These tools
adds features beyond even the new arrow (`+->+`) syntax in `switch`
expressions discussed in <<Updated `switch` syntax>>. Pattern matching
allows a `switch` label to be a type, not simply a value, and even
to add additional conditions to those labels.

Consider the following case where `bird` contains a reference of type `Bird`
and there are classes `Duck`, `Hoopoe`, `Owl`, and `Rooster` that are all
children of the `Bird` class.
...
This code would print out an appropriate bird noise, depending on the true
type of the object that `bird` points at. Note that the label after
each `case` must include the type to match against as well as a variable name.
In this example, we did not use variables like `duck` or `owl` on the right
side of the arrow to ask that object to call some method that only the matching
type could call, but we could have. In this way, a
pattern-matching `switch` works like an `instanceof` with a binding variable:
We get a variable with the right type to hold the object, without needing an
explicit cast. Note that a pattern-matching `switch` *must* handle every
possible type, so the `default` is required here.

Just like a `switch` with values for its `case` labels, a pattern-matching
`switch` can be used as an expression, storing its result into a variable. We
could change our bird example to store the result instead of printing it out.

[source,java]
----
String call = switch (bird) {
    case Duck duck -> "Quack!";
    case Hoopoe hoopoe -> "Oop-oop!";
    case Owl owl -> "Hoot!";
...
The following is a more practical example, a method that returns the area
of different shapes, depending on what type they are. It would be easy to add
additional `case` labels to support new shapes.
...
.Pitfall: Pattern matching with a parent class before a child class
[WARNING]
====

It appears as if execution of a pattern-matching `switch` jumps to the matching
`case` label, but it usually steps through each `case` in order. For this
reason, Java doesn't allow us to specify a parent class for a label before 
a child class. If it did, the child class label would never be reached since
the parent class would always match first.

Consider the following code that takes a reference called `creature` with
type `Animal` and prints out the name of the type that best matches it.
...
This code does *not* compile because the parent class `Animal` comes before the
child classes `Mammal` and `Human`. The appropriate way to write this code is
to order the classes in the opposite way, from child up to parent and finally
grandparent.
...
This behavior is exactly like `catch` blocks for exceptions, which must always
start from the most specific exception (the lowest-level child) and end with
the most generic (the highest-level parent), as we mentioned in 
<<Handling exceptions>>.
====
...
The main reason to use a `switch` statement is to cleanly list
possible alternatives and the code that should be executed for each one. To make
those alternatives as readable as possible, programmers prefer to write the
arrow syntax for `switch` statements with only a single line after the arrow.
Of course, multiple lines can be written after the arrow, if they're enclosed
in braces.

When an additional check with an `if` statement is needed inside a `case`,
braces could be used along with a `yield` statement, as in the following code
that checks for `String` values with an even length, `String` values whose
length is greater than 5, and `Duck` objects.
...
The value stored into `result` by this code is `"long string"`, since
`"unpredictable"` has a length of 13, which is not even but is longer than 5.
Nothing is wrong with this code, but it's almost as long as a version using
only `if` statements. Likewise, the `"who knows"` value must be repeated for
`String` values that are short with an odd length as well as objects that don't
match either the `String` or `Duck` types.

To handle this issue of conditional checks within a `case`, _guards_ can be
used. A guard is an expression that evaluates to a `boolean`, like the
condition of an `if` statement, that comes after an optional `when` keyword,
following the `case` label. A given `case` will match only if the object
matches the type and also makes the guard evaluate to `true`.

Thus, the longer code above could be written with guards in a more compact way
below.
...
The use of guards can allow a lot of logic to be compressed into a short space
while remaining readable. As you can see, guards can be applied to some `case`
labels and left off others, even within the same `switch`.

What happens if the object being tested by the `switch` statement is `null`?
Normally, using a `null` reference inside a `switch` will throw a
`NullPointerException`. However, a special `null` `case` can be added that will
...
In this code, a reference named `unknown` with type `Object` is the input to
the `switch` statement. If `unknown` points to `null`, this `switch` will give
`"(null)"` back as its result, instead of throwing an exception. A `null`
`case` doesn't follow the normal rules for `switch` statements, which typically
act as if they check each `case` label, one by one, from top to bottom.
Instead, a `null` `case` can be at the top, the bottom, or anywhere in between,
and the code for that `case` will always be executed if the reference is `null`.

The people who add features to Java continue to put a lot of effort into the 
updated arrow syntax for `switch` statements and the pattern-matching tools we've
discussed here. There are related features we did not mention as well
as new features that are likely to be standardized in future versions of Java,
including more flexible ways to use primitive types in `switch` constructs. If
you're interested in these features, you might want to search out information
about the latest versions of Java.


=== Solution: Banking account with a vengeance

Now, we return to the specific problem from the beginning of the
chapter and give its solution. We've already written the code for the
