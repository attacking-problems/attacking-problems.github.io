...
compile that class. If you remember all the required methods but
forget to specify that your class implements a particular interface, the
compiler will fail when you try to use that class in a place where the
...
default method was added to the rules for Java interfaces.  A method with a body _could_ be added to
...
interface.  But it's received two _different_ default methods for `tune()`, one that tunes like a guitarist and one that
...
without breaking classes that already implement those interfaces.
...
`Chord` and `Melody` defined, the following interface will compile. Note that
an `enum` might be a better way to define these constants, but we wanted to
emphasize how individual constants can be declared in an interface.
...
<<gui#gui>>. An elegant syntax for creating anonymous classes that
implement only a single method will also be discussed in
...
have the `Object` type. As we will discuss in <<The `Object` class>>, *any* 
object can be stored in a reference of type `Object`.
