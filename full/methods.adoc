...
For example, the following class includes a method called `increment()`
that increases the class variable `counter` every time it's called.

.Keeps track of the number of times the `increment()` method is called.
[source, java]
[[BookkeeperProgram]]
----
...
When run, this program calls the `increment()` method some random number of
times, and the variable `counter` keeps track of the number. Because `counter` is static,
it's accessible to both the `main()` and `increment()` methods.
...
=== Advanced: Variable numbers of arguments

In <<Overloaded methods>>, we discussed a way to create multiple methods with
the same name, for the convenience of the programmer. In our example, we created
a `max()` method that takes three arguments and returns the maximum of the
three, a method that coexists with (and even calls) the original `max()` method
that only takes two.

But what if we wanted to overload the `max()` method to take four, five, six, or
even a hundred arguments? It would be truly tedious to write hundreds of
versions of an overloaded method that fundamentally do the same thing. In Java 5
and higher, special syntax can allow a single method to take _any_
number of arguments. This syntax is usually referred to as _varargs_. Below is a
`max()` method that will work for any number of arguments greater than zero.
...
----
<.> The parameter `numbers` is marked as an array that contains a variable
number of arguments by putting three periods (`pass:[...]`) after the `int` type.
<.> Once declared in this way, `numbers` will behave exactly like any other
array, and all the `int` arguments provided in the call will be stored into it.

The following are all legal ways to call this special varargs version of the
`max()` function.

[source,java]
----
int largest1 = max(7); // 7
int largest2 = max(3, 8) // 8
int largest4 = max(9, 16, 11, -2) // 16
int[] array = {0, 4, 1, 5, 2};
int largestArray = max(array); // 5
----

The arguments can be provided by passing them in, as usual, separated by commas,
or you can also pass in a whole array at once. The varargs syntax is flexible
enough to accept either style.

However, the varargs syntax has several limitations as well. First, a method
can only have a single varargs parameter. Second, that parameter must be the
rightmost (last one listed). These restrictions are intended to avoid situations
where it might be unclear whether a given argument is part of a variable-length
list of arguments or not.

There are other dangers associated with varargs that are beyond what we can
cover here. Varargs can interact in potentially unsafe ways with generics, which
are discussed in <<dynamic-data-structures#dynamic-data-structures>>. For other
reasons, it's unwise to store data into the varargs array within the method or
to return it. Finally, note that as few as zero arguments can be passed into a
varargs parameter. The following is a legal method call, based on the definition
of `max()` given above.

[source,java]
----
int largest0 = max(); // Causes ArrayIndexOutOfBoundsException
----

This code compiles, but it will cause an error when it runs since it will try to
access the first element (at location 0) in an array that contains no elements.
There's no easy fix here. The programmer using this code should not try to find
the maximum of zero things, but programmers will often do unexpected things.
...
. [[varargsExercise]] Use varargs syntax to write a method that takes an
arbitrary number of `String` arguments and returns the one with the most vowel
characters in it. For the purposes of this exercise, a, e, i, o, and u (and
their uppercase versions) are considered vowels. If two `String` values have the
same number of vowels, return the one that comes earlier. Don't worry about the
case in which a user passes in no `String` arguments.
