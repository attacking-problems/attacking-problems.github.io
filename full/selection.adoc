...
_This section describes the `switch` statement as it was defined in versions before Java 12.  <<Updated `switch` syntax>> and <<_switch_expressions>>  describe enhancements that make `switch`
statements more readable and less error-prone and allow them to be used as an expression._
...
For example, imagine we want to create a program that determines
...
become unreadable and unmanageably wide.
...
to a case corresponding to the input. (The value for a `switch` can also have an
`enum` type, discussed in <<Advanced: Enumeration types>>.) We can recode the
anniversary gift example using a `switch` statement as follows.
...
be some kind of data that can be expressed as an integer, a `String` or an `enum`,
...
`default` label is unnecessary because execution will exit the `switch`
block anyway. Nevertheless, it's always wise to end on a `break`, in
the event that you add more cases later.
...
appropriately. Because of the dangers involved, it's often safer to
...
fall-through behavior can be useful. Imagine you need to write a
...
element of Chinese astrology is its zodiac, consisting of 12 animals.
...
The names of these animals are also sometimes translated in slightly
different ways.
...
==== Updated `switch` syntax

As mentioned earlier, the use of `break` statements can result in careless
errors if a `break` is accidentally left out or added where it isn't needed.  
New syntax introduced in Java 12 and made permanent in Java 14 makes it 
possible to write `case` statements that execution will jump to, execute, and 
then leave the `switch` block, without the need for a `break`.

Instead of a `case` being followed by a label, a colon, the code to execute, and
then a `break`, each `case` is followed by label, an arrow (`+->+`) and then a
single line of executable code. If `L` represents a label and `E` represents a
line of executable code, the syntax changes from `case L: E` to `+case L -> E+`.
Then, each `case` becomes a complete block,  and there is no fall-through to the
next `case`. To deal with many labels that all map to the same executable code,
these labels can be a list of values or constants separated by commas, as in the 
example below. If more than one statement is  included in a block, the statements
must be surrounded by curly braces (`+{ }+`).

Note that the original syntax for a `switch` statement *cannot* be mixed with the 
new syntax: All the cases within a single `switch` statement must either be of
the form `case L: E` or the form `+case L -> E+`, never both.

Here is an example that prints the number of days in the month given by its
three-letter abbreviation and uses a flag to indicate whether the month is
valid.
...
While there is a tremendous amount of legacy code that uses the older `switch`
style in which each `case` is followed by a single label and a colon, the newer
`+case L -> E+` style is preferred by many programmers. You should be aware of
the older style, but consider using the new style unless you need the unusual
fall-through behavior of an older-style `switch` that intentionally omits a
`break` statement or if you are following a style guide that insists on the
older style. 


==== `switch` expressions

The `switch` expression extends this new syntax further, producing a value that
can be used anywhere an expression is expected. Each `case` must compute a
value, and there are two ways to indicate the value being computed:

. Use the new `+case L -> E+` syntax. This syntax says that the resulting value
for `case L` is the value of expression `E`.  As before, a list of labels 
separated by commas can be used to map multiple labels to the same
expression.

. Use the new `yield E` statement.  The `yield` statement is similar to a 
`break` statement--it interrupts the flow of the `switch` statement--but also
indicates the value to be used as the result.

The `switch` expression can result in shorter code than a traditional `switch`
statement, making it easier to write and maintain.  The code below stores the
number of days in a given month, instead of printing it out like the previous
example.
...
Below is another example of a `switch` expression, which computes the
anniversary gift based on the year. Compare to the earlier statement version in
<<_switch_statements>>.

[source,java]
----
String gift = switch (year) {
    case 1 -> "Paper";
    case 2 -> "Cotton";
    case 3 -> "Leather";
    case 4 -> "Fruit";
...
Our last example of a `switch` expression uses a `yield` statement to indicate 
the result of the expression. It takes <<WesternZodiacProgram>> and changes the
`switch` statement with the old-style syntax into a `switch` expression.

[source, java]
[[WesternZodiacExpressionProgram]]
----
include::{programsdir}/WesternZodiacExpression.java[]
----

Most programmers use `yield` infrequently,
preferring the clear, readable style of putting the result value after the
arrow. However, when some computation is needed before the result is known, as
with the previous example, `yield` allows the contents of the `case` to contain
multiple lines.

.Pitfall: `switch` expression that doesn't cover all values
[WARNING]
====

Because a `switch` expression gives back a value, it *must* be _exhaustive_, 
meaning that it covers every possible value. Otherwise, if some input to the
`switch` has no matching `case`, the overall value of the `switch` cannot be
determined.

Consider the following code that records whether a `char` value called `letter`
is a vowel or a consonant.

[source,java]
----
String type = switch (Character.toLowerCase(letter)) {
    case 'a', 'e', 'i', 'o', 'u' -> "vowel";
    case 'b', 'c', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'p', 'q', 
    'r', 's', 't', 'v', 'w', 'x', 'y', 'z' -> "consonant";
};
----

This code does not compile because it does not cover cases in which `letter`
contains a character that is not a letter, such as `@`. Since it's impractical
(and tedious) to write out the thousands of possible characters that can be 
represented by a `char` value, using `default` is the best choice. The following
code compiles and works as intended.
