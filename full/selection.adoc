...
_This section describes the "basic" `switch` statement as it was defined in versions before Java 12.  <<_switch_expressions>> describes enhancements that allow the `switch` to be used anywhere an expression is used._
...
The `switch` expression computes a single value and can be used anywhere an expression is expected.  Each `case` must compute a single value.  There are two ways to indicate the value being computed:

. Use the new `yield E` statement.  The `yield` statement can be thought of as a `break` statement--it interrupts the flow of the `switch`--but also indicates the value to be used as the result.
...
The code below computes the number of days in a given month, with the month given as a three-letter abbreviation:
...
The `switch` expression can result in simpler code than the traditional `switch` statement, making it easier to write and maintain.  Below is a `switch` expression to compute the anniversary gift based on the year (compare to the earlier <<gift-switch-statement,statement version>>).
...
NOTE: It is possible to have a block of statements as the expression `E` in a `+case L -> E+` clause by including them in curly braces (`+{S1, S2, ..., Sn}+`).  In this case, at least one of the statements (typically `Sn`) is a `yield` statement that specifies the value of the `switch` expression.

NOTE: The `switch` statement and expression now allow the expression to be an object and the `case` clauses specify class names to match the object type.  A full discussion should be introduced with the `instanceof` operator.
...
. At the bottom of <<_basic_switch_statements>>, we use a
