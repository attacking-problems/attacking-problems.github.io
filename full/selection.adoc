...
.Pitfall: `switch` expression that doesn't cover all cases
[WARNING]
====

Because a `switch` expression gives back a value, it *must* be _exhaustive_, 
meaning that it covers every possible case. Otherwise, if some input to the
`switch` has no matching `case`, the value of the `switch` will not be known.
