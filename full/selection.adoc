...
_This section describes the `switch` statement as it was defined in versions before Java 12 and 14.  <<_switch_expressions>> and <<_switch_statements_without_break_statements>> describe enhancements that make `switch`
...
The `switch` expression, introduced in Java 12 and 14, computes a single value and can be used anywhere an expression is expected.  Each `case` must compute a value.  There are two ways to indicate the value being computed:

. Use the new `+case L -> E+` syntax.  This syntax says that the resulting value for `case L` is the value of expression `E`.  The label `L` can be a list of labels separated by commas, as in the first example below.
...
Below is another example of a `switch` expression, which computes the anniversary gift based on the year. Compare to the earlier statement version in <<_switch_statements>>.
...
the result of the expression. It takes <<WesternZodiacProgram>> and
changes the `switch` statement into a `switch` expression.
...
`+->+`. However, when some computation is needed before the result is known, as
with the last example, `yield` allows the contents of the case to contain
multiple lines.
...
As mentioned earlier, the use of `break` statements can result in careless errors: by accidentally omitting a `break` or adding one where it isn't needed.  With the introduction of the `+case L -> E+` syntax, it's possible to use a `switch` _statement_ without `break` statements at all.  Each `case` becomes a complete block, and there is no fall-through to the next case.  If more than one statement is included in a block, the statements must be surrounded by curly braces (`+{ }+`).

Note that the original syntax for a `switch` statement cannot be mixed with the 
new syntax: All the cases within a single `switch` statement must either be of the
form `case L:` or the form `+case L -> E+`, never both.
