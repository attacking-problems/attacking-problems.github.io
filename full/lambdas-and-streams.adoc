...
method, putting the contents of the data structure into a stream. Although you
...
Stream API is built heavily on generic types, they had to make some additions to
accommodate streams of generic types.
...
we give the starting point of 3 and then a lambda expression to add 1 to the
current value to get the next value. Without the `limit()` method, we would have
defined an infinite stream, but that's not a problem in and of itself.

In fact, streams use _lazy evaluation_. Usually, laziness is considered a
negative quality, but it can be a great thing in computer science. Lazy
evaluation means that a stream doesn't do computation on values in the stream
until it's asked to. Adding a long sequence of intermediate operations doesn't
mean that those operations will get performed on everything in stream. For
example, the `findFirst()` and `findAny()` methods are terminal operations that
will return only a single element from the stream. When using `findFirst()`,
it will transform the first element of the stream based on intermediate
operations, and if the first element matches, it will return that, without
considering other elements in the stream.
