...
Although it's possible to add even more ciphers, the last one we will add is the
Vigenère cipher.
...
<.> For testing purposes, we will encrypt
`"CRY HAVOC AND LET SLIP THE DOGS OF WAR"` from _Julius Caesar_ by William
Shakespeare.
<.> This `caesarCipher` object implements the Caesar cipher with a key of 7 that
will shift each letter of the alphabet seven places forward.
...
identical to the version that passing in `caesarCipher::encrypt`.
...
Since they're more readable, use method references when available. Nevertheless,
